---
audience: user
user-guide-title: Déploiements d’Adobe Experience
user-guide-description: Découvrez comment utiliser les déploiements d’Adobe Experience pour gérer les indicateurs de fonctionnalité, les déploiements contrôlés et les versions ciblées dans vos applications.
source-git-commit: db719ba7b9db91aea818d8ef216a28fcedc6aa65
workflow-type: tm+mt
source-wordcount: '340'
ht-degree: 5%

---


# Déploiements d’Adobe Experience {#experience-rollouts}

+ [Vue d’ensemble](home.md)
+ Prise en main {#get-started}
   + [Présentation des déploiements d’expérience](getting-started/introduction.md)
   + [Pourquoi utiliser les déploiements d’expérience ?](getting-started/why-use-experience-rollouts.md)
   + [Modes de déploiement d’expérience](getting-started/experience-rollouts-modes.md)
+ Concepts {#concepts}
   + [Qu’est-ce qu’un indicateur de fonctionnalité ?](concepts/what-is-a-feature-flag.md)
   + [Indicateurs de fonctionnalité pour activer et désactiver des fonctionnalités](concepts/feature-flags-to-enable-disable-features.md)
   + [Groupes de fonctionnalités pour contrôler plusieurs fonctionnalités](concepts/feature-groups-to-control-multiple-features.md)
   + [Concept de gestion des versions](concepts/concept-of-release-management.md)
   + [Gestion des versions dans les déploiements d’expérience](concepts/release-management.md)
   + [Déploiement graduel](concepts/gradual-rollout.md)
+ Guides {#guides}
   + Prise en main de la console {#console}
      + [Connectez-vous à la console Déploiements d’expérience](guides/console/log-in-to-the-console.md)
      + [Présentation des environnements](guides/console/environments-overview.md)
      + [Demander l’accès](guides/console/request-access.md)
      + [Équipes et leurs administrateurs](guides/console/teams-and-admins.md)
      + [Créer une nouvelle équipe](guides/console/create-a-new-team.md)
   + Équipes {#teams}
      + [Gestion des équipes](guides/teams/manage-teams.md)
      + [Rôles utilisateur](guides/teams/user-roles.md)
      + [Ajouter des membres à votre équipe](guides/teams/add-team-members.md)
      + [FAQ sur la gestion des équipes](guides/teams/team-management-faq.md)
   + Applications {#applications}
      + [Gestion des applications](guides/applications/manage-applications.md)
      + [Intégration de votre application](guides/applications/onboard-your-application.md)
   + Intégrer les déploiements d’expérience {#integrate}
      + [Intégration de déploiements d’expérience dans votre application](guides/integrate/integrating-in-your-app.md)
      + [Guide de démarrage](guides/integrate/startup-guide.md)
      + [Applications de bureau](guides/integrate/desktop-applications.md)
      + [Applications mobiles](guides/integrate/mobile-applications.md)
      + [Des applications web](guides/integrate/web-applications.md)
      + [Services Web](guides/integrate/web-services.md)
      + [SDK](guides/integrate/sdks.md)
      + [Étapes d’intégration](guides/integrate/integration-steps.md)
      + [Abonnement à l’application API dans Adobe Developer Console](guides/integrate/subscribe-to-api-application.md)
   + Indicateurs de fonctionnalités et versions {#feature-flags-releases}
      + [Fonctionnalités, groupes de fonctionnalités et versions](guides/feature-flags/features-feature-groups-releases.md)
      + [Créer votre premier indicateur de fonctionnalité](guides/feature-flags/create-your-first-feature-flag.md)
      + [Définir une fonctionnalité à déployer progressivement](guides/feature-flags/set-feature-gradual-rollout.md)
      + [Créer un groupe de fonctionnalités](guides/feature-flags/create-a-feature-group.md)
      + [Définir un groupe de fonctionnalités à déployer progressivement](guides/feature-flags/set-feature-group-gradual-rollout.md)
      + [Test A/B avec indicateurs de fonctionnalité](guides/feature-flags/a-b-testing.md)
      + [Versions et groupes de fonctionnalités interéquipes](guides/feature-flags/releases-and-cross-team-feature-groups.md)
      + [Workflow de publication complet](guides/feature-flags/release-workflow-end-to-end.md)
      + [Demander une version](guides/feature-flags/request-a-release.md)
      + [Mise à jour des règles d’audience de version](guides/feature-flags/update-release-audience-rules.md)
      + [États de la version](guides/feature-flags/release-states.md)
      + [Créer un groupe de fonctionnalités multi-équipes](guides/feature-flags/create-cross-team-feature-group.md)
      + [FAQ sur la gestion des versions](guides/feature-flags/release-management-faqs.md)
      + [Analytics](guides/feature-flags/analytics.md)
      + [Planning](guides/feature-flags/schedule.md)
   + Critères d’audience {#audience}
      + [Audience dans les indicateurs de fonctionnalité et les groupes de fonctionnalités](guides/audience/audience-in-feature-flags-and-feature-groups.md)
      + [Utilisation du contexte dans les règles d’audience](guides/audience/using-context-in-audience-rules.md)
      + [Règles d’audience complexes](guides/audience/complex-rules.md)
      + [Utilisation des données d’organisation d’entreprise dans les règles d’audience](guides/audience/using-enterprise-org-data.md)
      + [Ajouter des règles de pourcentage dans les critères d’audience](guides/audience/adding-percentage-rules.md)
      + [Règle d’audience avec variable de contexte IP client](guides/audience/clientip-rule.md)
   + Workflows interenvironnements {#cross-environment}
      + [Concept inter-environnements](guides/cross-environment/cross-environment-concept.md)
      + [Association d’environnements à une application](guides/cross-environment/associate-environments.md)
      + [Affichage des indicateurs de fonctionnalité dans les environnements](guides/cross-environment/view-feature-flags-across-environments.md)
      + [Importer les indicateurs de fonctionnalité](guides/cross-environment/import-feature-flags.md)
   + Support technique {#support}
      + [Résolution des problèmes](guides/support/troubleshooting.md)
      + [Obtenir de l’aide](guides/support/get-support.md)
      + [Contacter l’assistance](guides/support/contact-support.md)
   + Versions de SDK {#sdk-releases}
      + SDK Java {#java-sdk}
         + [Guide d’intégration de Java SDK](guides/sdk-releases/java/java-sdk-integration-guide.md)
         + [Notes de mise à jour de Java SDK](guides/sdk-releases/java/java-sdk-release-notes.md)
      + Node.js SDK {#nodejs-sdk}
         + [Guide d’intégration de Node.js SDK](guides/sdk-releases/nodejs/nodejs-sdk-integration-guide.md)
         + [Notes de mise à jour de Node.js SDK](guides/sdk-releases/nodejs/nodejs-sdk-release-notes.md)
      + [évaluation des performances de SDK](guides/sdk-releases/java-sdk-benchmarking.md)
+ API Feature {#feature-api}
   + [API de fonctionnalités GET V3](feature-api/get-feature-api-v3.md)
   + [API de fonctionnalités GET V2](feature-api/get-feature-api-v2.md)
+ API Management {#management-api}
   + [Présentation des API de gestion des fonctionnalités](management-api/feature-management-apis-overview.md)
   + [API Feature flags management](management-api/feature-flags-management-api.md)
   + [API Feature group Management](management-api/feature-group-management-api.md)
   + [API Release Management](management-api/release-management-apis.md)
   + [Obtention de l’identifiant client pour une application](management-api/get-client-id.md)
   + [Obtenir les critères d’audience souhaités](management-api/get-audience-criteria.md)
   + [API de correctif de gestion](management-api/management-patch-api.md)
