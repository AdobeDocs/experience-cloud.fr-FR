...
description: Découvrez comment importer des indicateurs de fonctionnalité d’un sandbox dans un autre dans les déploiements d’Adobe Experience pour éviter de recréer manuellement des configurations d’indicateur.
...
Les déploiements d’expérience vous permettent d’importer des indicateurs de fonctionnalité d’un sandbox (par exemple, le sandbox 1) dans un autre sandbox (par exemple, le sandbox 2). Cela évite d’avoir à recréer manuellement les configurations d’indicateurs et réduit le risque de dérive de la configuration entre les sandbox.
